...
<main x-data="data()">
  ...
  <div class="max-w-7xl px-4 sm:px-6 lg:px-12 mx-auto mt-4 forum-body">
    <div class="flex justify-between mb-5 items-center">
      <h1 class="text-lg font-bold text-gray-800">Modification de votre post <span class="text-gray-700"></span></h1>
      <button
      ...
      <form action="{{ route('posts.update', $post->id) }}" method="POST">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input name="title" value="{{ $post->title ?? old('title') }}"
          ...
          <div>
            ...
            <textarea class="block w-full rounded-md text-base py-2 h-40 border-gray-300" name="content" id="content"
              cols="30" rows="10">{{$post->content ?? old('content') }}</textarea>
            @error('content')
            <p class="text-red-600 mt-1 text-sm">{{ $message }}</p>
            @enderror
          </div>
